...
interface Visual1Props {
  ...
  gridColor?: string
  /** Datos reales de género */
  genderData?: { hombres: number; mujeres: number }
  /** Mensaje del dialog al hover */
  dialogTitle?: string
  dialogMsg?: string
}
...
export function Visual1({
  ...
  gridColor = "#80808015",
  genderData,
  dialogTitle = "Reporte del ítem",
  dialogMsg = "Sin datos comparativos disponibles.",
}: Visual1Props) {
  return (
    <div
      aria-hidden
    ...
    >
      ...
      <Layer3 color={mainColor} secondaryColor={secondaryColor} genderData={genderData} />
      <Layer4 title={dialogTitle} msg={dialogMsg} />
...
const Layer3 = ({ color, secondaryColor, genderData }: LayerProps & { genderData?: { hombres: number; mujeres: number } }) => {
  const label1 = genderData ? `H: ${genderData.hombres.toLocaleString('es-CO')}` : 'Hombre'
  const label2 = genderData ? `M: ${genderData.mujeres.toLocaleString('es-CO')}` : 'Mujer'
  return (
    <div
...
const Layer4 = ({ title, msg }: { title: string; msg: string }) => {
  return (
