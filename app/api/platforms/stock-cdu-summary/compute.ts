...
export interface StockCduSummary {
  ...
  top5Items: ItemStat[]
  top3Facultades: ItemStat[]
  top3Programas: ItemStat[]
...
export function computeStockCduSummary(
  loans: Loan[],
  items: Item[],
  ...
  for (const l of loans) {
    const f = l.facultad || 'Sin facultad'
    if (!facTotals[f]) facTotals[f] = { total: 0, prev: 0 }
    facTotals[f].total++
    if (sameYearMonth(l.loanDate, py, pm)) facTotals[f].prev++
  }
  const top3Facultades: ItemStat[] = Object.entries(facTotals)
    .map(([nombre, v]) => ({ nombre, total: v.total, totalMesAnterior: v.prev }))
    .sort((a, b) => b.total - a.total).slice(0, 3)

  // Top 3 programas
  const progTotals: Record<string, { total: number; prev: number }> = {}
  for (const l of loans) {
    const p = l.programa || 'Sin programa'
    if (!progTotals[p]) progTotals[p] = { total: 0, prev: 0 }
    progTotals[p].total++
    if (sameYearMonth(l.loanDate, py, pm)) progTotals[p].prev++
  }
  const top3Programas: ItemStat[] = Object.entries(progTotals)
    .map(([nombre, v]) => ({ nombre, total: v.total, totalMesAnterior: v.prev }))
    .sort((a, b) => b.total - a.total).slice(0, 3)

  // Por género y estamento
  const porGenero: Record<string, number> = {}
  const porEstamento: Record<string, number> = {}
  for (const l of loans) {
    const g = l.genero || 'No especificado'
    porGenero[g] = (porGenero[g] ?? 0) + 1
    const e = l.estamento || 'No especificado'
  ...
  return {
    ...
    prestamosMesActual, prestamosMesAnterior, top5Items, top3Facultades,
    top3Programas, itemsConDanos, prestamosVencidos, tasaDevolucion,
    porGenero, porEstamento, tendencia6Meses, rawDates,
  }
}
